...
| S5 | Medium | CSRF | Protection relied on `SameSite=Lax` alone, and Fastify also accepted `text/plain` bodies, which cross-site HTML forms can send. | **Fixed.** Only JSON bodies are accepted (other types get `415`). State-changing requests with a foreign `Origin` get `403 CROSS_ORIGIN_REJECTED` outside local development. In `NODE_ENV=development`, `@fastify/cors` is registered with `origin: true` and credentials enabled for local tooling, and the origin guard permits the request. Production ignores that override and uses `WEB_ORIGIN`; if it is empty, the `Origin` must match the request's `Host`. |
