...
- **OPEN DECISION 001** Choose the account identifier and credential policy: username or nickname, and password or PIN-equivalent. Define uniqueness, recovery, and age-appropriate constraints. *Resolved in [ADR-006](../decisions/ADR-006-credential-policy.md).*
...
- **OPEN DECISION 005** Define answer normalization for punctuation, capitalization, Unicode, placeholders, ellipses, and approximate answers. Approximate matching must not be enabled implicitly. *Resolved in [answer-matching.md](../architecture/answer-matching.md), including ä/ö/ü/ß transliterations.*
- **OPEN DECISION 006** Define the scoring formula, including repeated attempts in one session, incorrect-answer effects, and rounding or lower bounds. *Resolved in [scoring.md](../architecture/scoring.md).*
